...
double J1 = 0.0; //coupling constant for diagonal bonds
...
double Integrate(double C[TempCounts], float T_begin, float T_end, float T_steps)
	 { 
	   double entropy;
	   int i,t;
	   float h = T_steps, X;
	   int a = round(T_begin*int(1/T_steps));
	   int b = round(T_end*int(1/T_steps));
	    
	   double XI, XI1 = 0, XI2 = 0;
	   
     double XI0 = (C[a]/T_begin) + (C[b]/T_end);
...
{  
   ...
   //Temperature loop
   for(T=T_begin; T>T_end-T_steps; T = (round(int(1/T_steps)*(T - T_steps))/int(1/T_steps)))
   	{ 
  ...
  for(i=1; i<=TempCounts-1; i++)
  	{  
  	   ...
  	   Entropy = Integrate(C, tempT, T_end, T_steps);
