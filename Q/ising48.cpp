...
const int L = 48; //length of the lattice
...
double J1 = 0.85; //coupling constant for diagonal bonds
...
//Number of unhappy bonds on the square bonds
float Q()
 {
   int i,j, Uh_bonds = 0;
   ...
   for(i=0; i<L; i++)
	...
	for(j=0; j<L; j++)
	 {
	     if(Lattice[i][j]== Lattice[i][mod(j+1,L)]) Uh_bonds++;
	     if(Lattice[i][j]== Lattice[mod(i+1,L)][j]) Uh_bonds++;
	 }
      }
 
  return (float)Uh_bonds;
 }
...
{  
   ...
   ofstream file1, file2, file3, file4, file5, file6, file7, file8, file9, file10, file11, file12, file13, file14, file15;
   string fileName1,fileName2,fileName3,fileName4,fileName5,fileName6,fileName7,fileName8,fileName9, fileName10, fileName11,fileName12, fileName13, fileName14, fileName15;
   ...
   //intializing the file names 
    ...
    fileName12 = "Q_L="+to_string(L)+"_p=" + to_string(p)+ ".csv";
    fileName13 = "Q2_L="+to_string(L)+"_p=" + to_string(p)+ ".csv";
    fileName14 = "Qvar_L="+to_string(L)+"_p=" + to_string(p)+ ".csv";
    fileName15 = "Qstd_L="+to_string(L)+"_p=" + to_string(p)+ ".csv";
    ...
    file12.open(fileName12.c_str(), ios::out);
    file13.open(fileName13.c_str(), ios::out);
    file14.open(fileName14.c_str(), ios::out);
    file15.open(fileName15.c_str(), ios::out);
   ...
   float Q_sum=0, Q_sq_sum=0;
   float Q_avg=0, Q_sq_avg=0, Q_variance, Q_std_dev; 
  ...
  // double CorrE_avg, CorrE2_avg, Corr, CC, CC_avg;;
   ...
   //Temperature loop
   for(T=T_begin; T>0.01; T = (round(int(1/T_steps)*(T - T_steps))/int(1/T_steps)))
   	{ 
     ...
     Q_avg =  Q_sum/((float)monte_carlo_steps);
     Q_sq_avg = Q_sq_sum/((float)monte_carlo_steps);
    ...
    /* 
     ...
     Q_variance = (Q_sq_avg - Q_avg*Q_avg);
     Q_std_dev = sqrt(Q_variance);
